...
LCODE_FAILED = 3, //领取失败
      ...
      declare enum EM_PLATFORM {
        EPLAT_APHONE = 3, //安卓
EPLAT_IPHONE = 5, //苹果
      }
      

      declare enum EM_AD_TYPE {
        EM_AD_ACT = 1, //活动推广位
      }
...
// 推广位ID，读取优惠券包信息
      interface STReadCouponPackageReq {
        strActId?: string; //活动ID（废弃）
iAdPositionId?: number; //推广位ID
iPlatform?: number; //平台号
iAdType?: number; //推广类型，对应EM_AD_TYPE
strVActId?: string; //商城活动ID
strIPId?: string; //用于筛选的IPId
      }
